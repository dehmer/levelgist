...
const NodeFN = ({ M, put }) => {
  const root = Buffer.alloc(16)

  const Node = function (id, buf, root = false) {
    this.id = id
    this.buf = buf
    this.root = root
  }
  ...
  Node.prototype.write = async function () {
    await put(this.id, this.buf)
    if (this.root) await put(root, this.id)
    return this
  }

  Node.of = (id, entries = [], leaf = false, root = false) => {
    const buf = Buffer.allocUnsafe(size(M))
