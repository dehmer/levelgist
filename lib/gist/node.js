...
// Node.encode = (capacity, entries, leaf) => Buffer.from(JSON.stringify({
//   leaf,
//   entries: entries.map(entry => entry.decode())
// }))

Node.decode = (capacity, id, json) => {
  return Node.of(capacity, id, json.entries.map(Entry.encode), json.leaf)
}

Node.encode = (capacity, entries, leaf) => {
  return {
    leaf,
    entries: entries.map(entry => entry.decode())
  }
...
Node.prototype.encode = function () {
  return Node.encode(this.capacity_, this.entries_, this.leaf_)
}
