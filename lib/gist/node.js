 ...
 *
 */
const Node = function (id, entries, leaf) {
  this.id_ = id
...
Node.of = (id, entries = [], leaf = false) => {
  return new Node(id, entries, leaf)
}

Node.decode = (id, json) => {
  return Node.of(id, json.entries.map(Entry.encode), json.leaf)
}

Node.encode = (entries, leaf) => {
  return {
    leaf,
...
Node.prototype.encode = function () {
  return Node.encode(this.entries_, this.leaf_)
}
