 ...
 */
 const Insert = M => async function (E) {

  const insert = async (R, E) => {
    if (R.leaf()) {
      if (R.length() < M) return [await this.putNode(R.add(E))]
      else {
      ...
      if (nodes.length === 1) {
        ...
        // Update F MBR in R to MBR of nodes[0].
        F[0].setMBR(nodes[0].mbr())
        return [await this.putNode(R)]
      } else {
        const E = this.createEntry(nodes[1].mbr(), nodes[1].id())
        if (R.length() < M) {

          // Update F MBR in R to MBR of nodes[0] and
  ...
  const nodes = await insert(R, this.encodeEntry(E))
  if (nodes.length === 2) {
    const entries = nodes.map(N => this.createEntry(N.mbr(), N.id()))
    const root = this.createNode(this.key(), entries)
