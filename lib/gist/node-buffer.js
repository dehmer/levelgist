...
MBR.union = entries => {
  ...
  for (let i = 1; i < entries.length; i++) {
    if (entries[i].xmin_ < acc[0]) acc[0] = entries[i].xmin_
    if (entries[i].ymin_ < acc[1]) acc[1] = entries[i].ymin_
    if (entries[i].xmax_ > acc[2]) acc[2] = entries[i].xmax_
    if (entries[i].ymax_ > acc[3]) acc[3] = entries[i].ymax_
  }
 ...
 */
 const Entry = function (buf) {
  this.buf_ = buf
  this.xmin_ = this.buf_.readDoubleLE(OFFSET_XMIN)
  this.ymin_ = this.buf_.readDoubleLE(OFFSET_YMIN)
  this.xmax_ = this.buf_.readDoubleLE(OFFSET_XMAX)
  this.ymax_ = this.buf_.readDoubleLE(OFFSET_YMAX)
...
Entry.prototype.xmin = function () { return this.xmin_ }
Entry.prototype.ymin = function () { return this.ymin_ }
Entry.prototype.xmax = function () { return this.xmax_ }
Entry.prototype.ymax = function () { return this.ymax_ }
