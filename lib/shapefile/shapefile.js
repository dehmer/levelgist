...
const { UnderflowError } = require('./bytebuffer')
...
const point = (buffer, factory, next) => {
  ...
  if (shapeType === ShapeType.NULL) next(factory({ recordNumber, shapeType }))
    ...
    const [x, y] = [buffer.doubleLE(), buffer.doubleLE()]
    const box = { xmin: x, ymin: y, xmax: x, ymax: y }
    // skip possible Z/M values
    const remaining = length - 20
    buffer.skip(remaining)
    next(factory({ recordNumber, shapeType, box, points: [[x, y]] }))
  }
}
...
const multipoint = (buffer, factory, next) => {
  ...
  if (shapeType === ShapeType.NULL) next(factory({ recordNumber, shapeType }))
    ...
    buffer.skip(remaining)
    next(factory({ recordNumber, shapeType, box, points }))
  }
}
...
const poly = (buffer, factory, next) => {
  ...
  if (shapeType === ShapeType.NULL) next(factory({ recordNumber, shapeType }))
    ...
    const context = { shapeType, recordNumber, box, numParts, numPoints, hasM, hasZ }
